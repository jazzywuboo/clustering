...
sub GetArgs {
	...
	if ($#ARGV < 2 || $#ARGV > 3){
		print "Program requires 3 arguments; 4th argument is optional.\n";
		PrintUsageNotes();
		exit
	}

	if (!($reduced_ltc_file =~ /reduced/)){
		print "Error: $reduced_ltc_file is incorrect file. File exentsion should be _reduced.\n";
		PrintUsageNotes();
		exit
	}
	...
	return $base_dir, $reduced_ltc_file, $reduced_vector_file, $clustered_file, $num_clusters, $results_dir;
}
